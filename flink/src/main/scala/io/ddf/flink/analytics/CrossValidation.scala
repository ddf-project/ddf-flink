...
class RandomFilterFunction[T](seed: Long, lower: Double, upper: Double, isTraining: Boolean) extends RichFilterFunction[T] {
...
object CrossValidation {
  /** Return an Iterator of size k of (train, test) RDD Tuple
    ...
    */
  def randomSplit[T](dataset: DataSet[T], numSplits: Int, trainingSize: Double, seed: Long)(implicit _cm: ClassManifest[T]): Iterator[(DataSet[T], DataSet[T])] = {
  ...
  /** Return an Iterator of of size k of (train, test) RDD Tuple
    ...
    */
  def kFoldSplit[T](dataset: DataSet[T], numSplits: Int, seed: Long)(implicit _cm: ClassManifest[T]): Iterator[(DataSet[T], DataSet[T])] = {
    require(numSplits > 0)
   ...
   */
  private def getDDFCVSetsFromDataSets(splits: Iterator[(DataSet[_], DataSet[_])], manager: DDFManager, schema: Schema, nameSpace: String, unitType: Class[_]): JList[JList[DDF]] = {
    ...
    for ((train, test) <- splits) {
      ...
      val trainDDF = manager.newDDF(manager, train, Array(classOf[DataSet[_]], unitType), nameSpace, null, trainSchema)

      val testSchema = new Schema(null, schema.getColumns)
      val testDDF = manager.newDDF(manager, test, Array(classOf[DataSet[_]], unitType), nameSpace, null, testSchema)
