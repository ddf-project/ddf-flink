...
function ScanDirs {
  ...
  for jar in *.jar ; do
    ...
    if [ "mrql-core-0.9.6-incubating-SNAPSHOT.jar" == $jar ] ; then
      # We can't parse this generically because all the hyphens in the name
      GROUP_ID=com.adatao.unmanaged.org.apache.mrql ARTIFACT_ID=mrql-core VERSION=0.9.6-incubating-SNAPSHOT MvnInstall
      continue
    fi
    if [ "mrql-gen-0.9.6-incubating-SNAPSHOT.jar" == $jar ] ; then
      # We can't parse this generically because all the hyphens in the name
      GROUP_ID=com.adatao.unmanaged.org.apache.mrql ARTIFACT_ID=mrql-gen VERSION=0.9.6-incubating-SNAPSHOT MvnInstall
      continue
    fi
    if [ "mrql-flink-0.9.6-incubating-SNAPSHOT.jar" == $jar ] ; then
      # We can't parse this generically because all the hyphens in the name
